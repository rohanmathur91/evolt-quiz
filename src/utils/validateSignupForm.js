...
export const validateSignupForm = ({
  errorDispatch,
  credentials: { email, fullName, password, confirmPassword },
}) => {
  const isFullNameValid = fullName.length >= 3 && /^[a-zA-Z ]*$/.test(fullName);
  const isEmailValid = /[a-z0-9]+@[a-z]+\.[a-z]{2,3}/.test(email);
  const isPasswordValid =
    password !== "" &&
    /^(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,10}$/.test(password);
  ...
  if (!isPasswordValid) {
    errorDispatch({
      ...
      payload:
        "Password should contain atleast 6 characters, atleast one letter, a special character and a number",
    });
  }
